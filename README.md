...
- `get_paragraph_range`, `replace_text_in_range`, `delete_paragraph_range` — работать с диапазонами абзацев
- `insert_block_after_paragraph` — вставлять блоки (paragraph / heading / table) после опорного абзаца
- `batch_replace_text` и `batch_set_paragraph_format` — применять несколько правок за одно сохранение

### Таблицы, стили и секции
- `list_tables`, `get_table_cell_content`, `insert_table`, `update_table_cell`, `set_table_format`
- `batch_update_table_cells` — безопасно обновлять несколько ячеек таблиц за один проход
